...
class ProductAutoBarcode_1(models.Model):
    ...
    barcode_name = fields.Char('barcode_name', index=True, required=True, translate=True, size=16,compute="_compute_fieldvalue")

    @api.depends('name')
    def _compute_fieldvalue(self):
        for rec in self:
            rec.barcode_name = rec.name
